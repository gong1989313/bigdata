 ...
 * 快速排序(挖坑填数+分治法）
 * 
 * @author JiaJoa 通过查找一个待排序的序列中的一个基准位置（一般以第一个数为基准）， 使得基准左边的都小于这个基准，基准右边的都大于这个基，
...
public class AlgorithmQuickSort {
	public static void main(String[] args) {
		...
		int[] data = { 66, 13, 51, 76, 81, 26, 57, 69, 23 };
		AlgorithmQuickSort.quickSort(data);
	}
	...
	// 第二步，采用递归的方式处理基准左右两堆
	public static void quickRecursion(int low, int high, int[] data) {
		int base;
		if (low < high) {
			base = getPart(low, high, data);// 获取基准
			quickRecursion(low, base - 1, data);// 对低基准位段进行递归排序
			quickRecursion(base + 1, high, data);// 对高基准位段进行递归排序
		}
	}

	// 第三步，打印排序的序列
	public static void quickSort(int[] data) {
		quickRecursion(0, data.length - 1, data);
